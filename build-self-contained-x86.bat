...
chcp 65001 >nul
echo ========================================
echo Self-Contained Build for 32-bit Systems (x86)
echo ========================================
echo.
...
echo     ^<UseWindowsForms^>true^</UseWindowsForms^> >> WindowsSimpleCapture.csproj
...
echo     ^<ApplicationIcon^>icon.ico^</ApplicationIcon^> >> WindowsSimpleCapture.csproj
echo   ^</PropertyGroup^> >> WindowsSimpleCapture.csproj
echo. >> WindowsSimpleCapture.csproj
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo.
    echo Features: No .NET runtime required, runs directly on any Win10+ 32-bit system
) else (
    echo.
    echo ========================================
    echo Build Failed!
...
REM Restore original project file
